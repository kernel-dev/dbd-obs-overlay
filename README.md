...
FIRST, run the binary `server.exe` (or `python server.py`, whichever works). This will serve a local HTTP server so the application can read from `config.ini` (because OBS doesn't really support the `file://` protocol). 
